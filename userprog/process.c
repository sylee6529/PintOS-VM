 ...
 * Return true if successful, false if a memory allocation error
 * or disk read error occurs. */
/* 파일의 ofs 주소에서 시작하는 세그먼트를 메모리에 로드합니다.
 * ofs 주소에서 read_bytes만큼 읽고, 그 뒤부터 zero_bytes만큼은 0으로
 * 초기화한다. 이 함수에 의해 초기화된 페이지는 writable이 true이면 유저
 * 프로세스 사용 가능, 아니면 읽기 전용이다.
 */
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage,
                         uint32_t read_bytes, uint32_t zero_bytes,
                         bool writable) {
...
/* 스택 할당이 새 메모리 관리 시스템에 맞도록 첫번째 페이지로 바로 할당한다.(NOT
 * LAZY)  할당된 페이지는 스택(VM_MARKER_0)으로 표시한다. */
static bool setup_stack(struct intr_frame *if_) {
    bool success = false;
