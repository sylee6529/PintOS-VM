 ...
 * or disk read error occurs.
 * ---------------------------------------------------------------------
 * This function loads the file's content to the `upage`
 * Be awere that this function shouldn't load a file's content directly,
 * but hand over the ingredients for the actual loading such as function
 * and arguments to "lazy_load_segment()" */
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage,
                         uint32_t read_bytes, uint32_t zero_bytes,
                         bool writable) {
    ASSERT((read_bytes + zero_bytes) % PGSIZE == 0);
    ...
    while (read_bytes > 0 || zero_bytes > 0) {
        ...
        lazy_load_arg->ofs = ofs;  // the location will start to read
        lazy_load_arg->read_bytes =
            page_read_bytes;  // the bytes the page need to read
        lazy_load_arg->zero_bytes =
            page_zero_bytes;  // the bytes need to be filled with 0
...
/* Creates a page of stack at the point downwards as PGSIZE from USER_STACK
 * where the stack's start point. Note that the lazy loading is not necessary
 * for the first stack page. Becuase when the process is executed, this address
 * is accessed right away after this function is called in order to add
 * `commmand line args` to the stack. Therefore, we don't have to wait for the
 * fault, but get a physical frame right away. When a page is allocated,
 * suppliment marker can be used to indicate the page is stack page by using
 * `VM_MARKER_0` with the type. Lastly, change the rsp to `USER_STACK` so that
 * the `argument_stack` function can push the args from the rsp. */
static bool setup_stack(struct intr_frame *if_) {
    bool success = false;
