...
void test_max_priority(void) {
    if (!list_empty(&ready_list)) {
        ...
        if (!intr_context() &&
            priority_more(top_pri, &thread_current()->elem, NULL)) {
            thread_yield();
        }
    }
}
...
void thread_sleep(int64_t ticks) {
    ...
    old_level = intr_disable();
    if (curr != idle_thread) curr->wake_up_ticks = ticks;
...
void refresh_priority(void) {
    ...
    if (!list_empty(&thread_current()->donations)) {
        struct thread *front_thread =
            list_entry(list_begin(&thread_current()->donations), struct thread,
                       donations_elem);
...
void remove_with_lock(struct lock *lock) {
    struct list_elem *curr_donation_elem =
        list_begin(&thread_current()->donations);

    while (curr_donation_elem != list_tail(&thread_current()->donations)) {
        struct thread *curr_donation_thread =
            list_entry(curr_donation_elem, struct thread, donations_elem);
...
void mlfqs_load_avg(void) {
    ...
    int mult_load = mult_fp(a, load_avg);
